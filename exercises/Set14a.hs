...
greetText name
  | T.length name > 15 =
      T.concat [T.pack "Hello, ", T.take 15 name, T.pack "...!"]
  | otherwise =
      T.concat [T.pack "Hello, ", name, T.pack "!"]
...
longestRepeat txt =
  maximum (0 : map T.length (T.group txt))
...
pingpong b =
  BL.fromChunks (cycle [b, B.reverse b])
