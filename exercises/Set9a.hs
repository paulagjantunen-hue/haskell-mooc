...
workload nExercises hoursPerExercise =
  let total = nExercises * hoursPerExercise
  in if total > 100 then "Holy moly!"
    else if total < 10 then "Piece of cake!"
    else "Ok."
...
sumSuccess xs =
  let vals = [x | Right x <- xs]
  in if null vals then Left "no data"
    else Right (sum vals)
...
open c (Lock isOpen' code)
  | c == code = Lock True code
  | otherwise  = Lock isOpen' code

-- lock closes a lock. If the lock is already closed, nothing happens.
lock :: Lock -> Lock
lock (Lock _ code) = Lock False code
...
instance Eq Text where
  Text a == Text b =
    filter (not . isSpace) a == filter (not . isSpace) b
------------------------------------------------------------------------------
...
permute p xs =
  [ xs !! j | i <- [0 .. length xs - 1]
            , let Just j = lookup i (zip p [0..]) ]
