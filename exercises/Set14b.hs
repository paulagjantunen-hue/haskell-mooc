...
-- Database
import Database.SQLite.Simple (open,execute,execute_,query,query_,Connection,Query(..),Only(..))
...
balance db account = do
  rows <- query db balanceQuery (Only account) :: IO [Only Int]
...
parseCommand [cmd,name]
  | cmd == T.pack "balance" =
      Just (Balance name)

parseCommand [cmd,name,amountText]
  | cmd == T.pack "deposit" = do
      amount <- parseInt amountText
      return (Deposit name amount)

  | cmd == T.pack "withdraw" = do
      amount <- parseInt amountText
